...
CJ_RESULT CPPAReader::SetSyncParameters(uint8_t AddrByteCount, uint16_t PageSize,uint8_t Slot) {
...
RSCT_IFD_RESULT CPPAReader::ccidTransmit(uint8_t BWI, const uint8_t *cmd, uint16_t cmd_len,uint8_t *response,uint16_t *response_len,uint16_t wLevelParameter,uint8_t Slot) {
  ...
  if (Transfer(&Message,&Response)==CJ_SUCCESS) {
    if (Response.bMessageType!=RDR_TO_PC_DATABLOCK) {
      ...
      _IfdPower(SCARD_POWER_DOWN, NULL, NULL,0);
      *response_len=0;
			...
			if(Result==STATUS_SUCCESS || Result==STATUS_PARITY_ERROR || Result==STATUS_IO_TIMEOUT)
			{
				...
				if((sblock[1]&0xe0)==0xc0)
				{
					if(Result==STATUS_PARITY_ERROR || rbuffer[0]!=0 || (rbuffer[1]&0xe0)!=0xe0 || (rbuffer[1]&0xdf)!=sblock[1] || rbuffer[2]!=sblock[2] || memcmp(rbuffer+3,sblock+3,sblock[2])!=0)
					{
						if(++error>2)
						{
							_IfdPower(SCARD_POWER_DOWN,0,0,0);
...
RSCT_IFD_RESULT CPPAReader::_IfdTransmit(const uint8_t *cmd, uint16_t cmd_len,uint8_t *response,uint16_t *response_len,uint8_t Slot) {
 ...
 * - bPowerSelect is set to 5.0 Volts by default, but can be overwritten by Registry or ini-File
 */
RSCT_IFD_RESULT CPPAReader::_IfdPower(uint32_t Mode, uint8_t *ATR, uint32_t *ATR_Length, uint32_t Timeout,uint8_t Slot) {
  CCID_Message Message;
  CCID_Response Response;
  ...
  do
	...
	IFSC=m_p_Slot[Slot].m_TA3;
	PCB_seq=0;
	EDC=(unsigned char)(m_p_Slot[Slot].m_TC3&1);
...
RSCT_IFD_RESULT CPPAReader::IfdSetProtocol(uint32_t *pProtocol,uint8_t Slot) {
#if 0
  *pProtocol=m_ActiveProtocol;
  ...
  DEBUGP("PPAReader", DEBUG_MASK_COMMUNICATION_INFO,
         ...
         (int) Protocol,
         (int) m_p_Slot[Slot].m_ReaderState,
         (int) m_p_Slot[Slot].m_TA1,
         (int) m_p_Slot[Slot].m_TC1,
         (int) m_p_Slot[Slot].m_TC2,
         (int) m_p_Slot[Slot].m_TA3,
         (int) m_p_Slot[Slot].m_TB3,
         (int) m_p_Slot[Slot].m_TC3);
  ...
  *pProtocol=0;
  ...
  if (m_p_Slot[Slot].m_ReaderState==SCARD_NEGOTIABLE) {
	...
	vTA1=GetEnviroment(bufferTA1, m_p_Slot[Slot].m_TA1);
	vTC1=GetEnviroment(bufferTC1, m_p_Slot[Slot].m_TC1);
	bufferTC1[9]='2';
	m_p_Slot[Slot].m_TC2=GetEnviroment(bufferTC1, m_p_Slot[Slot].m_TC2);
	bufferTA1[8]='B';
	bufferTA1[9]='3';
	m_p_Slot[Slot].m_TB3=GetEnviroment(bufferTA1, m_p_Slot[Slot].m_TB3);
    ...
    case CJ_SUCCESS:
	  ...
	  if (m_p_Slot[Slot].m_ActiveProtocol==SCARD_PROTOCOL_T1)
                    ...
                    uint8_t *out,int out_len,uint8_t *in,int *in_len,uint8_t *Text,uint8_t Textlen,uint8_t bMessageIndex,uint8_t bNumberMessage,uint8_t Slot)
{
   ...
   memcpy(Message.Data.Secure.Data.Verify.abData,out,out_len);
    ...
    if(Res==CJ_SUCCESS && m_p_Slot[Slot].m_ActiveProtocol==SCARD_PROTOCOL_T1)
