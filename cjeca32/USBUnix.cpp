...
CBaseReader *CUSBUnix::BuildReaderObject() {
  ...
  dev=rsct_usbdev_scanDevByName(m_cDeviceName);
  if (dev==NULL) {
  ...
  if (dev->vendorId==AUSB_CYBERJACK_VENDOR_ID) {
    ...
    DEBUGP(m_cDeviceName, DEBUG_MASK_COMMUNICATION_ERROR, "Product: %d:%s", dev->productId, m_productString.c_str());
    ...
    r=_buildUsbReaderObject(dev->productId, (const char*)(m_productString.c_str()));
    rsct_usbdev_free(dev);
    return r;
  }
  else {
...
int CUSBUnix::Open() {
  ...
  nConfig=0;
  m_productString = dev->productName;
