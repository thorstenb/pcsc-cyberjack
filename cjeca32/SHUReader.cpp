...
CSHUReader::CSHUReader(CReader *Owner,CBaseCommunication *Communicator)
			  :base(Owner,Communicator)
{
}

CSHUReader::~CSHUReader(void)
{
}

void CSHUReader::SetHWString(char *String)
{
   strcpy(String,"SHU_");
}

void CSHUReader::GetProductString(uint8_t *Product)
{
	memcpy(Product,"CJSHU",5);
}

bool CSHUReader::ATRFilter(bool IsWarm,uint8_t Slot)
{
...
RSCT_IFD_RESULT CSHUReader::_IfdPower(uint32_t Mode, uint8_t *ATR, uint32_t *ATR_Length, uint32_t Timeout,uint8_t Slot)
{
...
uint32_t CSHUReader::GetReadersInputBufferSize()
{
	cj_ModuleInfo *Info;
...
bool CSHUReader::PinDirectSupported()
{
	return true;
}

CJ_RESULT CSHUReader::SpecialLess3_0_41()
{
	return CJ_SUCCESS;
}

RSCT_IFD_RESULT CSHUReader::IfdSetProtocol(uint32_t *pProtocol,uint8_t Slot)
{
	if(m_p_Slot[Slot].m_bIsRF)
	{
...
RSCT_IFD_RESULT CSHUReader::_IfdTransmit(const uint8_t *cmd, uint16_t cmd_len,uint8_t *response,uint16_t *response_len,uint8_t Slot)
{
...
CJ_RESULT CSHUReader::CtSetContrast(EContrast eContrast,uint32_t *Result)
{
	CJ_RESULT Res;
	switch(eContrast)
	{
...
RSCT_IFD_RESULT CSHUReader::IfdVendor(uint32_t IoCtrlCode,uint8_t *Input,uint32_t InputLength,uint8_t *Output,uint32_t *OutputLength)
{
	uint32_t Result;
...
CJ_RESULT CSHUReader::BuildReaderInfo()
{
	CJ_RESULT Res;
...
bool CSHUReader::HastModulestoreInfo()
{
	return true;
}

CJ_RESULT CSHUReader::CtSetSilentMode(bool boolMode,bool *pboolMode,uint32_t *Result)
{
	if(pboolMode)
		*pboolMode=boolMode;
  if(Result)
...
CJ_RESULT CSHUReader::CtGetSilentMode(bool *pboolMode,uint32_t *Result)
{
	if(pboolMode)
		*pboolMode=0;
  if(Result)
