...
int CCCIDReader::Transfer(CCID_Message *Message, CCID_Response *Response,uint8_t Slot)
{
	int Res;
	uint32_t Length;
	...
	{
		m_p_Slot[Slot].m_ActiveProtocol=0;
		m_p_Slot[Slot].m_ATR_Length=0;
	}
	if((Response->bStatus & 3)==2)
		m_p_Slot[Slot].m_ReaderState=SCARD_ABSENT;
...
RSCT_IFD_RESULT CCCIDReader::IfdGetState(uint32_t *State,uint8_t Slot)
{
	...
	*State=SCARD_UNKNOWN;
	...
	if(Transfer(&Message,&Response,Slot)==CJ_SUCCESS)
	{
		...
		{
		case 0:
			*State=m_p_Slot[Slot].m_ReaderState;
			break;
		case 1:
			*State=SCARD_SWALLOWED;
			m_p_Slot[Slot].m_ReaderState=SCARD_SWALLOWED;
			break;
		default:
			*State=SCARD_ABSENT;
			m_p_Slot[Slot].m_ReaderState=SCARD_ABSENT;
...
RSCT_IFD_RESULT CCCIDReader::IfdSetProtocol(uint32_t *pProtocol,uint8_t Slot)
{
	CCID_Message Message;
	CCID_Response Response;
	memset(&Message,0,sizeof(Message));
	...
	*pProtocol=0;
	...
	if(m_p_Slot[Slot].m_ReaderState==SCARD_NEGOTIABLE)
	{
		char bufferTA1[128];
		char bufferTC1[128];
		char bufferc[3];

		if(m_p_Slot[Slot].m_ATR[0]==0xff || (m_p_Slot[Slot].m_ATR[0] & 0xf0)==0x80)
		{
		...
		sprintf(bufferTA1,"ReplaceTA1_%02X",(int)m_p_Slot[Slot].m_TA1);
		strcpy(bufferTC1,"ReplaceTC1_");
		for(unsigned int i=0;i<m_p_Slot[Slot].m_ATR_Length;i++)
		{
			sprintf(bufferc,"%02X",(int)(m_p_Slot[Slot].m_ATR[i]));
...
/*		if((Protocol & (SCARD_PROTOCOL_T0 | SCARD_PROTOCOL_T1)==(SCARD_PROTOCOL_T0 | SCARD_PROTOCOL_T1))
		...
		if((Protocol & SCARD_PROTOCOL_T0) && (m_p_Slot[Slot].m_PossibleProtocols & SCARD_PROTOCOL_T0))
		{
			Message.dwLength=5;
			Message.Header.SetParameters.bProtocolNum=0;
			Message.Data.SetParameters.T0.bGuardTimeT0=(uint8_t)GetEnviroment(bufferTC1, m_p_Slot[Slot].m_TC1);
			Message.Data.SetParameters.T0.bmFindexDindex=(uint8_t)GetEnviroment(bufferTA1, m_p_Slot[Slot].m_TA1);
			Message.Data.SetParameters.T0.bWaitingIntegerT0=m_p_Slot[Slot].m_TC2;
		}
		else if((Protocol & SCARD_PROTOCOL_T1) && (m_p_Slot[Slot].m_PossibleProtocols & SCARD_PROTOCOL_T1))
		{
			Message.dwLength=7;
			Message.Header.SetParameters.bProtocolNum=1;
			Message.Data.SetParameters.T1.bGuardTimeT1=(uint8_t)GetEnviroment(bufferTC1, m_p_Slot[Slot].m_TC1);
			Message.Data.SetParameters.T1.bmFindexDindex=(uint8_t)GetEnviroment(bufferTA1, m_p_Slot[Slot].m_TA1);
			Message.Data.SetParameters.T1.bWaitingIntegerT1=m_p_Slot[Slot].m_TB3;
			Message.Data.SetParameters.T1.bIFSC=m_p_Slot[Slot].m_TA3;
		}
		else
		{
		...
		switch(Transfer(&Message,&Response,Slot))
		{
		case CJ_SUCCESS:
			if((Response.bStatus&3)==2)
				return STATUS_NO_MEDIA;
			...
			else
			{
				...
				{
					m_p_Slot[Slot].m_ActiveProtocol=SCARD_PROTOCOL_T0;
				}
				else
				{
	...
	else if(m_p_Slot[Slot].m_ReaderState==SCARD_SPECIFIC)
	{
		if(Protocol & (SCARD_PROTOCOL_DEFAULT | SCARD_PROTOCOL_OPTIMAL))
...
int CCCIDReader::ctBcsReset(uint8_t *atr,uint8_t *atr_len,uint8_t *historical,uint8_t *hist_len,uint8_t prevered,int Timeout,uint8_t Slot)
{
	...
	*atr_len=0;
	...
	switch(_IfdPower(SCARD_COLD_RESET,atr,&len,Timeout,Slot))
	{
	case STATUS_SUCCESS:
		switch(IfdSetProtocol(&Protocol,Slot))
		{
		case STATUS_SUCCESS:
			*atr_len=(uint8_t)len;
			memcpy(historical,m_p_Slot[Slot].m_Historical,m_p_Slot[Slot].m_Historical_Length);
			*hist_len=(uint8_t)m_p_Slot[Slot].m_Historical_Length;
   		return CJ_SUCCESS;
...
bool CCCIDReader::HasDiversifier()
{
	return false;
}
		...
		*sad=1;
		...
		else if(cmd[0]==0x20)
		{
			...
			if(cmd[1]==0x10)  /*B1*/
			{
				...
				if(cmd[2]==0)
				{
					for(i=0;i<m_maxSlot;i++)
				     IfdPower(SCARD_POWER_DOWN,0,0,i);
				}
				else
				   IfdPower(SCARD_POWER_DOWN,0,0,cmd[2]-1);

				if(cmd[2]>=1 && cmd[2]<=m_maxSlot)
				{
					uint8_t atr[250];
					uint8_t historical[15];
					uint8_t hist_len=sizeof(historical);
					uint8_t atr_len=(uint8_t)sizeof(atr);
					switch(ctBcsReset(atr,&atr_len,historical,&hist_len,0,0,cmd[2]-1))
					{
					case CJ_SUCCESS:
						if(cmd[3]==0)
						{
			...
			else if(cmd[1]==0x11)
			{
				...
				if(cmd[2]>=1 && cmd[2]<=m_maxSlot && cmd[3]>0 && (Lc!=-1 || Le!=0))
				{
					response[0]=0x67;
					response[1]=0x00;
					*lenr=2;
...
//				Sleep(3000);
			...
			else if(cmd[1]==0x12)
			{
				...
				int Timeout=0;
				cmd[3]&=0x0f;
				if(cmd[2]<1 || cmd[2]>m_maxSlot || cmd[3]>2)
				{
					response[0]=0x6A;
					response[1]=0x00;
					*lenr=2;
				...
				if(IfdGetState(&State,cmd[2]-1)==STATUS_DEVICE_NOT_CONNECTED)
				{
					*lenr=0;
					return -128;
				}
				...
				}
				{
					...
					uint8_t atr_len=(uint8_t)sizeof(atr);
					Timeout*=1000;
					switch(ctBcsReset(atr,&atr_len,historical,&hist_len,0,Timeout,cmd[2]-1))
					{
					case CJ_SUCCESS:
						if(cmd[3]==0)
						{
			...
			{
				if(lenc==4 && cmd[2]>=0x01 && cmd[2]<= m_maxSlot && cmd[3]==0x00)
				{
					if(*lenr<8)
					{
						*lenr=0;
					...
					response[3] = 0x72;
					response[4] = 0x01;
					switch(m_p_Slot[cmd[2]-1].m_ActiveProtocol)
					{
				...
				if(cmd[2]>m_maxSlot || (cmd[2]==0 && cmd[3]!=0x46 && cmd[3]!=0x80 && cmd[3]!=0x81) ||
					(cmd[2]>=1 && cmd[2]<=m_maxSlot && cmd[3]!=0x80))
				{
					response[0]=0x6A;
					response[1]=0x00;
					*lenr=2;
				...
				if(cmd[3]==0x80)
				{
					...
					if(cmd[2]==0)
					{
  					uint8_t i;
						...
						for(i=0;i<m_maxSlot;i++)
						{
							if(IfdGetState(&State,i)==STATUS_DEVICE_NOT_CONNECTED)
							{
								*lenr=0;
								return -128;
							}
						...
						response[2+m_maxSlot]=0x90;
						response[3+m_maxSlot]=0x00;
						*lenr=4+m_maxSlot;
					}
					else
					{
			...
			else if(cmd[1]==0x14)
			{
				...
				if(cmd[2]<1 || cmd[2]>m_maxSlot || cmd[3]!=0)
				{
					response[0]=0x6A;
					response[1]=0x00;
					*lenr=2;
				...
				if(IfdPower(SCARD_POWER_DOWN,0,0,cmd[2]-1)!=STATUS_SUCCESS)
					return -10;
				if(m_p_Slot[cmd[2]-1].m_ReaderState!=SCARD_ABSENT)
				{
					response[0]=0x90;
					response[1]=0x00;
				}
			...
			else if(cmd[1]==0x15)
			{
				...
				if((cmd[3]&0x04)!=0x04 && lenc>4)
				{
					...
					if((tag80=GetTag(data_ptr,Lc,0x80,&taglen))!=NULL || Lc==1)
					{
						...
						_IfdPower(SCARD_POWER_DOWN,0,0,Timeout,cmd[2]-1);
...
/*						if(((cjccidHANDLE)hDevice)->PCB_seq&0x01)
						...
						switch(Res=((use_MV==0)?cjccid_SecurePV(timeout,PinPosition,PinType,
							...
							Min,Max,
							Condition,Prologue,
							buffer,outlen,rbuffer,&inlen,tag50[0],taglen50[0],bMessageIndex,bNumberMessage,cmd[2]-1):
							...
							Condition,Prologue,
							0,PinPosition-1,
							buffer,outlen,rbuffer,&inlen,TextCount,tag50,btaglen50,bMessageArray,bNumberMessage,NULL,cmd[2]-1)))
						{
						case CJ_SUCCESS:
...
//					MessageBox(NULL,"Bitte Leser updaten.\nPlease update your reader.","Warnung - Warning",MB_ICONSTOP | MB_OK);
				...
				if(Le==0 && HasDiversifier() && (tag80=GetTag(data_ptr,Lc,0x83,&taglen))!=NULL)
				{
					if(taglen==4)
						pDiversifier=tag80;
				}
				if((Lc<9 || (Le!=-1 && pDiversifier==NULL)) || (Lc<15 && pDiversifier!=NULL))
				{
					response[0]=0x67;
					response[1]=0x00;
					*lenr=2;
...
/*						if(((cjccidHANDLE)hDevice)->PCB_seq&0x01)
						...
						switch(Res=cjccid_SecureMV(timeout,PinPosition,PinType,
							...
							buffer,outlen,rbuffer,&inlen,TextCount,tag50,btaglen50,bMessageIndex,bNumberMessage,pDiversifier,cmd[2]-1))
						{
						case CJ_SUCCESS:
						case CJ_ERR_PIN_EXTENDED:
/*							if(((cjccidHANDLE)hDevice)->Protokoll==1)
			...
			else if(cmd[1]>=0x70 && cmd[1]<=0x7f && IsClass3() && FindModuleWithMask(MODULE_ID_KT_LIGHT,0xfffffeff))
			{
		...
		else if(cmd[0]==0x80)				
		{
			if(cmd[1]==0x60)
			{
				uint8_t *tag44;
				int taglen;
				if(cmd[2]<0x01 || cmd[2]>m_maxSlot || cmd[3]!=0x00)
				{
					response[0]=0x6A;
					response[1]=0x00;
				...
				if((tag44=GetTag(cmd+5,cmd[4],0x44,&taglen))!=NULL)
				{
					...
					if((*tag44 & 0xf0)==0x90)
					{
						m_p_Slot[cmd[2]-1].m_iic_offset_bytes=2;
					}
					else
					{
						m_p_Slot[cmd[2]-1].m_iic_offset_bytes=1;
					}
					m_p_Slot[cmd[2]-1].m_iic_pagesize=1<<(*tag44 & 0x0f);
					SetSyncParameters(m_p_Slot[cmd[2]-1].m_iic_offset_bytes,m_p_Slot[cmd[2]-1].m_iic_pagesize,cmd[2]-1);
				}
				if((tag44=GetTag(cmd+5,cmd[4],0x45,&taglen))!=NULL)
				{
					...
					m_p_Slot[cmd[2]-1].m_iic_deviceaddr=*tag44;
				}
				if((tag44=GetTag(cmd+5,cmd[4],0x22,&taglen))!=NULL)
				{
					...
					if(m_p_Slot[cmd[2]-1].m_ActiveProtocol != SCARD_PROTOCOL_T0 && m_p_Slot[cmd[2]-1].m_ActiveProtocol != SCARD_PROTOCOL_T1)
					{
						response[0]=0x90;
						response[1]=0x00;
						*lenr=2;
					...
					if(m_p_Slot[cmd[2]-1].m_ActiveProtocol==SCARD_PROTOCOL_T0 && *tag44!=1 || m_p_Slot[cmd[2]-1].m_ActiveProtocol==SCARD_PROTOCOL_T1 && *tag44!=2)
					{
						...
						switch(ctBcsReset(atr,&atr_len,historical,&hist_len,*tag44,0,cmd[2]-1))
						{
						case CJ_SUCCESS:
							if(m_p_Slot[cmd[2]-1].m_ActiveProtocol==SCARD_PROTOCOL_T0 && *tag44==1 || m_p_Slot[cmd[2]-1].m_ActiveProtocol==SCARD_PROTOCOL_T1 && *tag44==2)
							{
...
/*			else if((((cjccidHANDLE)hDevice)->Info.Flags & 1)==0 && cmd[1]==0x61 && cmd[2]==0x82 && cmd[3]==0  && cmd[4]==0)
	...
	else if((*dad!=1 && *dad<=m_maxSlot) && *sad==2)						
	{
		*sad=*dad;
		*dad=2;
		if(*lenr<2)
		...
		switch(_IfdTransmit(cmd,lenc,response,lenr,(*sad>1)?(*sad-1):0))
		{
		case STATUS_SUCCESS:
				return 0;
		case STATUS_IO_TIMEOUT:
		...
		case STATUS_INVALID_DEVICE_STATE:
			if(m_p_Slot[(*sad>1)?(*sad-1):0].m_ReaderState==SCARD_ABSENT)
			{
				*sad=1;
				response[0]=0x64;
				response[1]=0xA1;
                    ...
                    uint8_t *out,int out_len,uint8_t *in,int *in_len,uint8_t *Text,uint8_t Textlen,uint8_t bMessageIndex,uint8_t bNumberMessage,uint8_t Slot)
{
   ...
   Info=FindModule(MODULE_ID_KT_LIGHT_GC);
	...
	Res=Transfer(&Message,&Response,Slot);
#ifdef _INSERT_KEY_EVENTS
	if(Info!=NULL && Info->Status==MODULE_READY)
...
#endif
	CompressModifyStructure(Message);
	Res=Transfer(&Message,&Response,Slot);
	if(Res!=0)
	{
#ifdef _INSERT_KEY_EVENTS
...
#else
		if(InputLength!=sizeof(PIN_VERIFY_STRUCTURE)-sizeof(((PIN_VERIFY_STRUCTURE*)NULL)->abData)+((PIN_VERIFY_STRUCTURE *)Input)->ulDataLength)
#endif
         return STATUS_INVALID_BUFFER_SIZE;
...
#else
		if(InputLength!=sizeof(PIN_MODIFY_STRUCTURE)-sizeof(((PIN_MODIFY_STRUCTURE*)NULL)->abData)+((PIN_MODIFY_STRUCTURE *)Input)->ulDataLength)
#endif
         return STATUS_INVALID_BUFFER_SIZE;
...
RSCT_IFD_RESULT CCCIDReader::IfdVerifyPinDirect(PIN_VERIFY_STRUCTURE *Input,uint8_t *Output,uint32_t *OutputLength)
{
   ...
   else if(m_p_Slot[0].m_ActiveProtocol==SCARD_PROTOCOL_T1)
...
void CCCIDReader::CompressModifyStructure(CCID_Message& Message)
{}
