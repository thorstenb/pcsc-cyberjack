...
RSCT_IFD_RESULT CECRReader::_IfdPower(uint32_t Mode, uint8_t *ATR, uint32_t *ATR_Length, uint32_t Timeout,uint8_t Slot)
{
	uint32_t OrginalATR_Length=0;
	if(ATR_Length!=0)
		OrginalATR_Length=*ATR_Length;
	RSCT_IFD_RESULT Result=base::_IfdPower(Mode,ATR,ATR_Length,Timeout);
	m_p_Slot[Slot].m_bIsRF=false;
	if(Result==STATUS_SUCCESS && Mode==SCARD_COLD_RESET || Mode==SCARD_WARM_RESET)
	{
		if(m_p_Slot[Slot].m_ATR_Length>=6 && m_p_Slot[Slot].m_ATR_Length<=12)
		{
			uint8_t tck=0;
			if(m_p_Slot[Slot].m_ATR[0]==0x41)
			{
				bool bDoSelectTCL=true;

				m_p_Slot[Slot].m_SerialNumberLength=m_p_Slot[Slot].m_ATR_Length-3;
				memcpy(m_p_Slot[Slot].m_SerialNumber,m_p_Slot[Slot].m_ATR+3,m_p_Slot[Slot].m_SerialNumberLength);
				if(m_p_Slot[Slot].m_ATR[1] & 0x20)
				{
					if(m_p_Slot[Slot].m_ATR[1]==0x28 || m_p_Slot[Slot].m_ATR[1]==0x38)
					{
					...
					if(bDoSelectTCL)
					{
          			...
          			Message.Header.SetParameters.bProtocolNum=3;//Get ATS
						...
						case CJ_SUCCESS:
							...
							memcpy(m_p_Slot[Slot].m_ATR,"\x3b\x80\x80\x01",4);
							...
							m_p_Slot[Slot].m_ATR[1]|=Response.Data.abData[0];
							memcpy(m_p_Slot[Slot].m_ATR+4,Response.Data.abData+3+i,Response.Data.abData[0]);
							m_p_Slot[Slot].m_ATR_Length=Response.Data.abData[0]+4;
				...
				if(!bDoSelectTCL)
				{
					uint8_t merkSAK=m_p_Slot[Slot].m_ATR[1];
				   memcpy(m_p_Slot[Slot].m_ATR,"\x3b\x8f\x80\x01\x80\x4f\x0c\xA0\x00\x00\x03\x06\x00\x00\x00\x00\x00\x00\x00",19);
					m_p_Slot[Slot].m_ATR_Length=19;
					switch(merkSAK & 0xdf)
					{
					case 0x08:
						memcpy(m_p_Slot[Slot].m_ATR+12,"\x03\x00\x01",3);
						break;
					case 0x09:
						memcpy(m_p_Slot[Slot].m_ATR+12,"\x03\x00\x26",3);
						break;
					case 0x18:
						memcpy(m_p_Slot[Slot].m_ATR+12,"\x03\x00\x02",3);
						break;
					default:;
					}
				}
				for(unsigned int i=1;i<m_p_Slot[Slot].m_ATR_Length;i++)
				{
					tck^=m_p_Slot[Slot].m_ATR[i];
				}
				m_p_Slot[Slot].m_ATR[m_p_Slot[Slot].m_ATR_Length]=tck;
				m_p_Slot[Slot].m_ATR_Length++;
				if(OrginalATR_Length<m_p_Slot[Slot].m_ATR_Length)
...
//   		        m_ReaderState=SCARD_NEGOTIABLE;
			...
			else if(m_p_Slot[Slot].m_ATR[0]==0x42)
			{
				m_p_Slot[Slot].m_SerialNumberLength=4;
				memcpy(m_p_Slot[Slot].m_SerialNumber,m_p_Slot[Slot].m_ATR,4);
				memmove(m_p_Slot[Slot].m_ATR+4,m_p_Slot[Slot].m_ATR+1,8);
				memcpy(m_p_Slot[Slot].m_ATR,"\x3b\x88\x80\x01",4);
				m_p_Slot[Slot].m_ATR_Length=12;
				for(unsigned int i=1;i<m_p_Slot[Slot].m_ATR_Length;i++)
				{
					tck^=m_p_Slot[Slot].m_ATR[i];
				}
...
RSCT_IFD_RESULT CECRReader::IfdSetProtocol(uint32_t *pProtocol,uint8_t Slot)
{
...
RSCT_IFD_RESULT CECRReader::_IfdTransmit(const uint8_t *cmd, uint16_t cmd_len,uint8_t *response,uint16_t *response_len,uint8_t Slot)
{
	int l;
	...
	if(m_p_Slot[Slot].m_bIsRF && cmd_len==5 && cmd[0]==0xff && cmd[1]==0xca && cmd[2]<=0x01 && cmd[3]==0)
	{
		if(cmd[2]==0)
		{
			if(*response_len>=m_p_Slot[Slot].m_SerialNumberLength+2)
			{
				if(cmd[4]==0 || cmd[4]>=m_p_Slot[Slot].m_SerialNumberLength)
				{
				   memcpy(response,m_p_Slot[Slot].m_SerialNumber,m_p_Slot[Slot].m_SerialNumberLength);
					if(cmd[4]!=0 && cmd[4]>m_p_Slot[Slot].m_SerialNumberLength)
					{
				      memset(response+m_p_Slot[Slot].m_SerialNumberLength,0,cmd[4]-m_p_Slot[Slot].m_SerialNumberLength);
				      memcpy(response+cmd[4],"\x62\x82",2);
					   *response_len=cmd[4]+2;
					}
					else
					{
				      memcpy(response+m_p_Slot[Slot].m_SerialNumberLength,"\x90\x00",2);
					   *response_len=m_p_Slot[Slot].m_SerialNumberLength+2;
					}
					return STATUS_SUCCESS;
				}
			}
			...
			{
			   response[0]=0x6C;
			   response[1]=m_p_Slot[Slot].m_SerialNumberLength;
				*response_len=2;
				return STATUS_SUCCESS;
			}
   		return STATUS_BUFFER_TOO_SMALL;
