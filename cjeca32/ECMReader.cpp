...
RSCT_IFD_RESULT CECMReader::_IfdPower(uint32_t Mode, uint8_t *ATR, uint32_t *ATR_Length, uint32_t Timeout,uint8_t Slot)
{
	return CECRReader::_IfdPower(Mode,ATR,ATR_Length,Timeout);
}

RSCT_IFD_RESULT CECMReader::IfdSetProtocol(uint32_t *Protocol,uint8_t Slot)
{
	return CECRReader::IfdSetProtocol(Protocol);
}

RSCT_IFD_RESULT CECMReader::_IfdTransmit(const uint8_t *cmd, uint16_t cmd_len,uint8_t *response,uint16_t *response_len,uint8_t Slot)
{
