...
RSCT_IFD_RESULT CECAReader::_IfdPower(uint32_t Mode, uint8_t *ATR, uint32_t *ATR_Length, uint32_t Timeout, uint8_t Slot)
{
//	cj_ModuleInfo *pInfo;
	switch(Mode)
	{
...
/*		pInfo=FindModule(MODULE_ID_KERNEL);
			 ...
			 (memcmp(m_ReaderInfo.TestDate+3,"05",2)==0 && memcmp(m_ReaderInfo.TestDate,"03",2)<=0)))
		{
			...
			return base::_IfdPower(Mode,ATR,ATR_Length,Timeout);
	default:
		return base::_IfdPower(Mode,ATR,ATR_Length,Timeout);
	}
}
