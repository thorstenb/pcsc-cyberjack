...
CJ_RESULT CReader::Disonnect()
{
	...
	if(m_Reader)
	{
		for(uint8_t i=0;i<m_Reader->m_maxSlot;i++)
		   m_Reader->IfdPower(SCARD_POWER_DOWN,0,0,i);
		m_Reader->Unconnect();
	   delete m_Reader;
	}
	m_Reader=NULL;
...
CJ_RESULT CReader::CtSelfTest2(const uint8_t *TransportKey,uint8_t KeyNr)
{
	CJ_RESULT res;
	if(m_Reader==NULL)
	{
	...
	CritSec->Enter();
	res=m_Reader->CtSelfTest2(TransportKey,KeyNr);
	CheckcJResult(res);
	CritSec->Leave();
	return res;
}
...
void CReader::SetChangeInterruptCallback(fctChangeIntCallback ChangeIntCallback,ctxPtr ChangeOwner)
{
	...
	CritSec->Leave();
	if(ChangeIntCallback && m_Reader!=NULL)
	{
    m_Reader->IfdGetState(&State);
		ChangeIntCallback(ChangeOwner,(State==SCARD_ABSENT)?0:1);
	}
